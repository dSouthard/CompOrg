...
unsigned long long address, cacheStartAddress, byteStartAddress, byteEndAddress, cacheEndAddress; // Read in new addresses
...
int main(int argc, char* argv[]) {
    ...
    unsigned int l1_hit_time = 1, l1_miss_time = 1;
    unsigned int l2_hit_time = 5, l2_miss_time = 7;
    unsigned int l2_transfer_time = 5, l2_bus_width = 16; // L1 -> L2 access penalties
    ...
    if (mem_chunksize <= 16) {
        mainMemoryCost = 50 + 25;
    } else if (mem_chunksize == 32) {
        mainMemoryCost = 50 + 125;
    } else {
        mainMemoryCost = 50 + 225;
    }

    // Main memory access penalty
    unsigned int mainMemoryTime = mem_sendaddr + mem_ready + (mem_chunktime * 64 / mem_chunksize);
    ...
    while (scanf("%c %Lx %d\n", &opCode, &address, &byteSize) == 3) {
        ...
        while (fscanf(inputFile, "%c %Lx %d\n", &opCode, &address, &byteSize) != EOF) {
            ...
            unsigned long long traceTime = 0; // track the time this trace took
            ...
            while (bytesLeft > 0) { // Go through and repeat for as many bytes as necessary
                ...
                if (opCode == 'I') {
                    ...
                    if (hit == 1) { // Found in L1I cache
                        ...
                        if (writeOverTag) {
                            ...
                            if (scanCache(l2Cache, l2WritebackTag, l2WritebackIndex, 'W')) { // Be seen as a write
                                ...
                                traceTime += l2_transfer_time * (iCache->blockSize / l2_bus_width); // Move desired block from L1 -> L2
                    ...
                    if (hit == 1) { // Found in L1I cache
                        ...
                        if (writeOverTag) {
                            ...
                            if (scanCache(l2Cache, l2WritebackTag, l2WritebackIndex, 'W')) { // Be seen as a write
                                ...
                                traceTime += l2Cache->hitTime;
                                traceTime += l2_transfer_time * (dCache->blockSize / l2_bus_width); // Move desired block from L1 -> L2
                            } else {
                    ...
                    if (hit == 1) { // Found in L1I cache
                        ...
                        if (debug) printf("Now scanning L2 for target tag\n"
                                "\t *** L2: Index Field = %llx, Tag Field = %llx *** \n", l2IndexField, l2Tag);
                        if (scanCache(l2Cache, l2Tag, l2IndexField, 'R')) { // Hit in L2, not dirty there
                            if (debug)
                                printf("\t HIT: Found in L2, adding L2 hit time (+%d)\n", l2_hit_time);
                            l2HitCounter++; // Increment hit counter
                            traceTime += l2Cache->hitTime;
            ...
            executionTime += traceTime; // Add trace time to the total execution time
            if (debug) {
...
void printHelpStatement() {
    printf("Memory Simulation Project, ECEN 4593 \n"
            "Usage: cat <traces> | ./main <config_file> -FLAGS\n"
...
void printReport() {
    ...
    printf("Execution Time = %llu, \tTotal Traces Read = %llu\n", executionTime, traceCounter);
    ...
    if (instructionCounter)
        insCycles = ((double) (readTime + writeTime + instructionTime)) / ((double) instructionCounter);
