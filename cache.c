...
/********************* Read Trace Function ***************************/
unsigned long long readTrace(struct Cache* cache, unsigned long long targetTag, unsigned long long targetIndex, char op){
    int i=0, j=0;					// Used for iteration
    struct Block *tempBlock;	// Used for LRU policy implementation
    
    // Go through blockArray, try to find targetTag. Start at most-recently used block in targetIndex
	...
	while (tempBlock.nextBlock != null) {
		if (tempBlock.valid) {	// Only check blocks if they are valid
			...
			if (tempBlock.tag == targetTag) {
				  ...
				  cache->blockArray[targetIndex][j].(*nextBlock).dirty=1;
				  cache->writeTime+=hitTime;	// increase write time by hit penalty
				} 
				
				else if (op == 'R') { // valid targetTag found in cache, read operation
					cache->readTime+=hitTime;	// increase read time by hit penalty
				}
				
				else if (op == 'I') { // valid targetTag found in cache, instruction operation
					cache->instructionTime+=hitTime;	// increase instruction time by hit penalty
				}
				    
				// Move block to beginning of LRU chain
		  		struct Block *dummyPointer = cache->blockArray[targetIndex][0].nextBlock;	// Keep track of current start of chain
		  		cache->blockArray[targetIndex][0].nextBlock = tempBlock;					// Make this referenced block the new start of chain
		  		tempBlock = tempBlock.nextBlock;											// Point to where referenced block was pointing
		  		struct Block *dummyPointer2 = cache->blockArray[targetIndex][0].nextBlock;
		  		dummyPointer2->nextBlock = dummyPointer;
		  		while (dummyPointer.nextBlock != cache->blockArray[targetIndex][0].nextBlock) {
		  			dummyPointer = dummyPoint->nextBlock;		// Go through chain until you find the block that was pointing to the referenced block
		  		}
		  		dummyPointer.nextBlock = tempBlock;				// Change that block to reconnect the chain
		      	          	
		      	// Done
		      	return 0;
			}	// end matching targetTag if
		
		// targetTag wasn't found, look into next block
		tempBlock = tempBlock.nextBlock;
		}
	}
	
	// Reached a null block => reached the end of the LRU chain without a hit
	
		
	
	// Find the end of the LRU chain, send its tag back to be replaced
